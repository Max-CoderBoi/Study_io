...
// Initialize Groq (FREE tier, no credit card needed!)
...
export async function askQuestion(question, namespace = "default") {
  try {
    ...
    if (!process.env.GROQ_API_KEY) {
      throw new Error("GROQ_API_KEY not configured. Get it from console.groq.com");
    }
    ...
    // Generate response using Groq (SUPER FAST!)
...
Please provide your answer now:`
    ...
    // Safely extract the answer with error checking
    ...
    if (error.message.includes("quota") || error.message.includes("limit")) {
      return "The AI service quota has been exceeded. Please check your API quota or try again later.";
    }
    
    if (error.message.includes("401") || error.message.includes("unauthorized")) {
      return "Invalid API key. Please check your GROQ_API_KEY from console.groq.com";
    }
